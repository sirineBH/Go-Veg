...
angular.module('myApp.pancacke', ['myApp.services', 'myApp.recipeCard'])

    .controller('pancakeController', ['$scope', 'api', function ($scope, api) {
        $scope.next = false;
        $scope.loading = false;
        $scope.firstLoading = true;
        $scope.recipes = [];

        api.getRecipes("Pancake").then(function (response) {
