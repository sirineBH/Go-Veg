...
angular.module('myApp.salad', ['myApp.services', 'myApp.recipeCard'])

    .controller('saladController', ['$scope', 'api', function ($scope, api) {
        $scope.next = false;
        $scope.loading = false;
        $scope.firstLoading = true;
        $scope.recipes = [];

        api.getRecipes("Salad").then(function (response) {
