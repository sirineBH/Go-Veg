...
angular.module('myApp.desserts', ['myApp.services', 'myApp.recipeCard'])

    .controller('dessertsController', ['$scope', 'api', function ($scope, api) {
        $scope.next = false;
        $scope.loading = false;
        $scope.firstLoading = true;
        $scope.recipes = [];

        api.getRecipes("Desserts").then(function (response) {
