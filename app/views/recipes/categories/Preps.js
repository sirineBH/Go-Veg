...
angular.module('myApp.preps', ['myApp.services', 'myApp.recipeCard'])

    .controller('prepsController', ['$scope', 'api', function ($scope, api) {
        $scope.next = false;
        $scope.loading = false;
        $scope.firstLoading = true;
        $scope.recipes = [];

        api.getRecipes("Preps").then(function (response) {
